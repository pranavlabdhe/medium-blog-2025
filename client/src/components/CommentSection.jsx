...
export default function CommentSection({ postId }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch('https://medium-blog-2025.onrender.com/api/comment/create', {
        method: 'POST',
        headers: {
  ...
  useEffect(() => {
    const getComments = async () => {
      try {
        const res = await fetch(`https://medium-blog-2025.onrender.com/api/comment/getPostComments/${postId}`);
        if (res.ok) {
          const data = await res.json();
          setComments(data);
        }
